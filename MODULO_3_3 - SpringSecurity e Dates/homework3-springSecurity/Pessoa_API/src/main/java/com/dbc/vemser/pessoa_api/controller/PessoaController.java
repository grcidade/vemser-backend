...
public class PessoaController {
    ...
    public List<PessoaEntity> listByData(@DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate dataInicial,
        ...
        return pessoaRepository.findByDataNascimentoBetween(dataFinal, dataInicial);
    }
    ...
    public List<PessoaEntity> findByDate(@DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate valor1,
                                         @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate valor2){
        return pessoaRepository.findByDataNascimentoBetween(valor1, valor2);
    }

    @GetMapping("/getIfHasAddress")
    public List<PessoaEntity> findIfHasAddress(){
        return pessoaRepository.findIfHasAddress();
    }

    @GetMapping("/getWithoutAddress")
    public List<PessoaEntity> listarPessoasQueNaoPossuemEndereco() {
        return pessoaRepository.getPessoasSemEndereco();
    }

    @GetMapping("/pessoa-completa")
    public List<PessoaDTOComContatosEEnderecos> listarPessoasComEnderecosEContatos()throws Exception {
        return pessoaService.getPessoaCompleta();
    }
